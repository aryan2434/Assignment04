window.onload = function () {
  ...
  if (productName) {
    const productDetailsApiUrl = `https://makeup-api.herokuapp.com/api/v1/products.json?product_name=${encodeURIComponent(
      productName
    )}`;
    ...
    fetch(productDetailsApiUrl)
        ...
        if (productDetails.length > 0) {
          ...
          productDetailsElement.innerHTML = `
                        ...
                        <strong>Description:</strong> ${
                          firstProduct.description || "Not available"
                        }<br>
                    `;
        } else {
      ...
      .catch((error) => {
        ...
        productDetailsElement.innerText =
          "Error fetching product details. Please try again.";
      });
  } else {
